...
namespace jest
{
  ...
  {
    expect_equal(color_coded::conf::load("", "c"),
                 color_coded::conf::defaults("c"));
    expect_equal(color_coded::conf::load("", "c++"),
                 color_coded::conf::defaults("c++"));
    expect_equal(color_coded::conf::load("", "objective-c"),
  ...
  {
    expect_equal(color_coded::conf::load("nonexistent/file", "c"),
                 color_coded::conf::defaults("c"));
    expect_equal(color_coded::conf::load("nonexistent/file", "c++"),
                 color_coded::conf::defaults("c++"));
    expect_equal(color_coded::conf::load("nonexistent/file", "objective-c"),
                 color_coded::conf::defaults("objective-c"));
    expect_equal(color_coded::conf::load("nonexistent/file", "objective-c++"),
                 color_coded::conf::defaults("objective-c++"));
  }
  ...
  template <>
  void
  color_coded::config_group::test<4>() /* Prefer language specific config. */
  {
  ...
  template <>
  void
  color_coded::config_group::test<5>() /* compilation database, valid file. */
  {
    auto const args(
        color_coded::conf::load(compile_commands, "c++", test_file));
    expect(std::find(args.begin(), args.end(), "-I/etc/") != args.end());
    expect(std::find(args.begin(), args.end(), "-foo") != args.end());
  ...
  template <>
  template <>
  void color_coded::config_group::test<6>() /* compilation database, header
                                               without flags. */
  {
    auto const args_source(
        color_coded::conf::load(compile_commands, "c++", test_file));
    auto const args_header(
        color_coded::conf::load(compile_commands, "c++", test_header));
    auto const args_include(
        color_coded::conf::load(compile_commands, "c++", test_include));
  ...
  template <>
  template <>
  void color_coded::config_group::test<7>() /* compilation database,
                                               non-existent file defaults. */
  {
    expect_equal(color_coded::conf::load("compile_commands.json",
                                         "c++"
                                         "nonexistent/file"),
                 color_coded::conf::defaults("c++"));
  }
  ...
  template <>
  template <>
  void color_coded::config_group::test<8>() /* Constants still present with
  ...
  template <>
  template <>
  void color_coded::config_group::test<9>() /* Pre/post constants in the right
                                               positions. */
  {
    auto const args(color_coded::conf::load(test_config, "c++"));
    ...
    expect(std::find(pre_constants.begin(),
                     pre_constants.end(),
                     *args.begin()) != pre_constants.end());
    // The last arg should be a post-constant.
    expect(std::find(post_constants.begin(),
                     post_constants.end(),
                     *(args.end() - 1)) != post_constants.end());
  }
} // namespace jest
