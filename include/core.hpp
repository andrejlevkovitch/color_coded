...
namespace color_coded
{
  namespace core
  {
    ...
    inline auto &queue()
    {
      static async::queue<async::task, async::result> q{[](async::task const
                                                               &t) {
        try {
          ...
          if (config_it != config_paths.end() && config_it->second.empty()) {
            ...
            config_it = config_paths
                            .insert({t.name,
                                     conf::load(conf::find(t.name, t.filetype),
                                                t.filetype,
                                                t.name)})
                            .first;
          }
          ...
          conf::args_t const &config_args_impl{config_it->second};
          ...
          return async::result{t.name, {trans_unit, std::move(tp)}};
