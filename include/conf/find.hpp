...
namespace color_coded
{
  namespace conf
  {
    ...
    inline fs::directory_iterator find_file(fs::path const &   dir,
                                            std::string const &file)
    {
      static fs::directory_iterator const end;
      return std::find_if(
          fs::directory_iterator{dir}, end, [&](fs::directory_entry const &e) {
    ...
    inline std::string find(fs::path file, std::string const &filetype)
    {
      ...
      std::string const flag_files[]{
          typed_file_name, compilation_database, file_name};
