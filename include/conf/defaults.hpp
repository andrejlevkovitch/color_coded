  ...
  namespace conf
  {
    ...
    inline args_t pre_constants(std::string const &filetype)
    {
      ...
      if (filetype == "c") {
        ...
        std::move(
            cpp_includes.begin(), cpp_includes.end(), std::back_inserter(args));
        return args;
    ...
    inline args_t post_constants()
    {
      return {
          ...
          environment<env::tag>::clang_resource_dir, // internal libraries and
                                                     // intrinsics
          "-isystem/usr/include",
#ifdef __APPLE__
...
#endif
          "-w",
          "-fcolor-diagnostics" // See
                                // https://github.com/jeaye/color_coded/issues/104
      };
    }

    /* Add system defaults to user specified arguments. Needed because libclang
     * often fails to find system search paths. */
    inline args_t add_defaults_to_args(std::string const &filetype,
                                       args_t &&          args)
    {
      ...
      std::copy(std::begin(post_additions),
                std::end(post_additions),
